...
export const useSettingsStore = defineStore(
  ...
  },
  {
    persist: {
      ...
      omit: ["system"],
      afterHydrate: ({ store }) => {
        const { lyric, player, appearance } = store as unknown as {
          lyric: LyricSettings;
        ...
        if (
          player &&
          (player.outputDevice === "diretta:undefined" ||
            player.outputDevice === "undefined" ||
            player.outputDevice === "diretta:null" ||
            player.outputDevice === "system-default")
        ) {
          player.outputDevice = null;
        }
